 ...
 * @author Touraj
 */
public class SceinceDirectProcessor implements IProcessor{
    
    public StringBuffer sb;

    public SceinceDirectProcessor() {
        sb = new StringBuffer();
    }
    ...
    public void buildQueryByXpath(NodeList nodes, String entity) {
        ...
        for (int i = 0; i < nodes.getLength(); ++i) {
            ...
            if (aggregationType.equalsIgnoreCase("main")) {
                sb.append(entity+"(");
                sb.append(nodes.item(i).getFirstChild().getNodeValue());

            } else if (aggregationType.equalsIgnoreCase("and")) {
                sb.append(" ");
                sb.append(nodes.item(i).getFirstChild().getNodeValue());
            }
            else if (aggregationType.equalsIgnoreCase("or")) {
                sb.append(" or ");
                sb.append(nodes.item(i).getFirstChild().getNodeValue());
            }
